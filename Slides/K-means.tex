...
\frame{\frametitle{Parallel version}
\justifying
Note that "upgradeCentroids()" isn't parallelized. That is because different data points might be added to the sum of them to calculate the centroid at the same time, leading to Write-After-Write (WAW) hazard.\vspace{1cm}
...
In this way mutual exclusion is guaranteed. In the first part of the algorithm we set 8 threads, while the second part the number is equal to K.
}
